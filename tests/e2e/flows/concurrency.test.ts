...
describe("Concurrency (E2E)", () => {
  ...
  describe("Session Concurrency", () => {
    test("handles multiple sessions from same wallet", async () => {
      const wallet = createTestWallet();

      // Login first time to create user
      ...
      const login1 = await api.post("/auth/login").send({
      ...
      // Login second time (user already exists)
      ...
      const login2 = await api.post("/auth/login").send({
        walletAddress: wallet.publicKey,
        signature: sig2,
        nonce: nonce2,
      });
  ...
  describe("Stress Test", () => {
    ...
    test.skip("handles burst of 20 concurrent orders without data corruption", async () => {
      ...
      // Verify order count matches SOL balance
      const ordersRes = await api
        .get("/orders")
        .set("Authorization", `Bearer ${token}`);
      ...
      // SOL should match successful orders (each order is 0.1 SOL)
      expect(finalSol).toBeCloseTo(orderCount * 0.1, 5);

      // USDC = 1000 - (orderCount * $10) - (orderCount * $0.01 fee)
      const expectedUsdc = 1000 - orderCount * 10 - orderCount * 0.01;
      expect(finalUsdc).toBeCloseTo(expectedUsdc, 1);

      // Balances should never go negative
      expect(finalUsdc).toBeGreaterThanOrEqual(0);
      expect(finalSol).toBeGreaterThanOrEqual(0);
    });
  });
});
